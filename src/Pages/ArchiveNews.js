...
const ArchiveNews = () => {
  ...
  return (
    <div className="news-page archive-page">
      <div className="news-main-content">
        <div className="news-main-info">
          <p>
            Вы здесь: <Link to="/">Главная</Link> /{" "}
            <Link to="/news">Новости</Link> / Архив
          </p>
          <h1>Архив новостей — {getMonthName(month)} {year}</h1>

          {filteredNews.length === 0 ? (
            <p className="no-news-message">Нет новостей за выбранный период.</p>
          ) : (
              ...
              {filteredNews.map((item) => {
                ...
                return (
                  <div className="news-card-vertical" key={item.NewsID}>
                    ...
                    <p>{shortDesc}</p>
                    <a className="view-button" href={`/news/${item.NewsID}`}>
                      Читать далее
                    </a>
                  </div>
                );
